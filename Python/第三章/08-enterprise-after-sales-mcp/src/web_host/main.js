 ...
 * 企业售后 Web Host。
 *
 * 浏览器只访问 Python Web Host，不直接持有 DeepSeek Key，
 * 也不直接连接带 Bearer Token 的 MCP Server。
 */
...
function finishConfirmation(accepted) {
	...
	resolve?.(accepted ? 'accept' : 'decline')
}
...
async function callModel() {
	const response = await postJson('/api/model', {
		messages,
		tools: openAiTools()
	})
...
async function requestMcpTool(name, args) {
	...
	if (response.kind === 'elicitation') {
		const decision = await showConfirmation(response.message)
...
async function callMcpTool(name, args, { renderApp = true } = {}) {
	...
	try {
		...
		if (renderApp && getToolUiResourceUri(tool)) {
...
function closeMcpApps() {
	for (const cleanup of appCleanups.splice(0)) cleanup()
}

/**
 * 使用 MCP Apps 标准 JSON-RPC 消息完成初始化，并发送 Tool Result。
 *
 * Python 负责提供 Resource；浏览器端这段少量 JavaScript 负责 iframe
 * 隔离和 ui/initialize 握手，不需要安装 Node 或构建前端包。
 */
async function renderMcpApp({ tool, args, result }) {
	...
	try {
		...
		await new Promise((resolve, reject) => {
			const timeout = window.setTimeout(() => {
				cleanup()
				reject(new Error('MCP App 初始化超时'))
			}, 5000)
			...
			const listener = (event) => {
				...
				if (message?.method === 'ui/notifications/size-changed') {
					...
					if (height) {
						iframe.style.height =
							`${Math.min(Math.max(height, 280), 900)}px`
					}
				}
			}
...
questionElement.addEventListener('input', () => {
	...
	questionElement.style.height =
		`${Math.min(questionElement.scrollHeight, 140)}px`
})
