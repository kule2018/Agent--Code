...
INCIDENT_SCENARIOS = {
    "release-regression": {
        ...
        "metrics": {
            ...
            "summary": "5xx 和响应延迟在 15:10 同时升高，CPU、内存和数据库连接池仍处于正常范围。",
        },
        "logs": {
            ...
            "summary": "主要错误来自 currency 字段读取失败，首次出现时间为 15:10，错误实例运行版本为 v2.4.1。",
        },
        "deployments": {
            "items": [
                {
    ...
    "database-pool": {
        ...
        "metrics": {
            ...
            "summary": "数据库连接池使用率达到 100%，同时出现大量请求等待，CPU 和内存没有明显异常。",
        },
        "logs": {
            "window": "2026-07-27 15:08:00 - 15:18:00",
            "totalErrorCount": 604,
            ...
            "summary": "主要错误是获取数据库连接超时，没有出现空指针、版本兼容或网络连接错误。",
        },
        "deployments": {
            "items": [
                {
        ...
        "databasePool": {
            ...
            "summary": "连接池已经耗尽，47 个请求正在等待；对账报表聚合查询长期占用连接。",
        },
    },
}


def get_incident_scenario(scenario_name):
    """根据命令行场景名读取故障数据。"""

    scenario = INCIDENT_SCENARIOS.get(scenario_name)

    if scenario is None:
        options = "、".join(INCIDENT_SCENARIOS.keys())
        raise ValueError(f"未知场景 {scenario_name}。可选值：{options}")
