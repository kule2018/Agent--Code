...
function findLatestDiff(trace: TraceEvent[]) {
	const event = [...trace]
		...
		.find(
			(item) =>
				item.toolName === 'get_git_diff' &&
				typeof item.data?.patch === 'string'
		)
...
<template>
	<div class="app-shell">
		...
		<div class="workspace-shell">
			<aside :class="['sidebar', { 'sidebar-open': sidebarOpen }]">
				...
				<section class="new-run-panel">
					...
					<div :class="['provider-note', { available: capabilities?.ai.available }]">
					...
					>
						<span :class="['run-icon', `status-${run.status}`]">
							<component
								...
								:class="{ spin: run.status === 'running' || run.status === 'planning' }"
								:size="15"
							/>
						</span>
						<span class="run-copy">
							...
							<small>
								{{ run.mode === 'ai' ? 'AI' : 'Replay' }} · {{ formatTime(run.createdAt) }}
							</small>
						</span>
						<ArrowRight :size="14" />
					</button>
			...
			<main class="main-area">
				...
				<template v-if="selectedRun">
					<section class="run-header">
							...
							<div class="run-eyebrow">
								<span :class="['status-badge', `status-${selectedRun.status}`]">
									<component
										...
										:class="{ spin: selectedRun.status === 'running' || selectedRun.status === 'planning' }"
										:size="13"
									/>
								...
								<span :class="['mode-badge', `mode-${selectedRun.mode}`]">
									...
									{{ selectedRun.mode === 'ai' ? selectedRun.model ?? 'AI' : 'Replay' }}
								</span>
						...
						<button
							v-if="selectedRunIsActive && selectedRun.status !== 'waiting_approval'"
					...
					<section class="metrics-strip">
						<div class="metric">
							<div class="metric-label">
								...
								<span>{{ selectedRun.mode === 'ai' ? '模型调用' : 'Replay 轮次' }}</span>
							</div>
							<strong>
								{{ selectedRun.mode === 'ai' ? selectedRun.usage.modelCalls : selectedRun.usage.iterations }}
						...
						<div class="metric">
							<div class="metric-label"><TerminalSquare :size="16" /><span>工具调用</span></div>
							<strong>{{ selectedRun.usage.toolCalls }} / {{ selectedRun.limits.maxToolCalls }}</strong>
							<div class="meter">
								<i :style="{ width: meterWidth(selectedRun.usage.toolCalls, selectedRun.limits.maxToolCalls) }" />
							</div>
						</div>

						<div class="metric">
							<div class="metric-label"><FileCode2 :size="16" /><span>文件变化</span></div>
							<strong>{{ selectedRun.usage.filesChanged }} / {{ selectedRun.limits.maxFilesChanged }}</strong>
							<div class="meter">
								<i :style="{ width: meterWidth(selectedRun.usage.filesChanged, selectedRun.limits.maxFilesChanged) }" />
							</div>
						</div>

						<div class="metric verification-metric">
							<div class="metric-label"><TestTube2 :size="16" /><span>验证</span></div>
							<strong :class="verificationLabelClass(selectedRun)">
								{{ verificationLabel(selectedRun) }}
							</strong>
							<div class="verify-pills">
								<span :class="stateClass(selectedRun.verification.testsPassed)">TEST</span>
								<span :class="stateClass(selectedRun.verification.typecheckPassed)">TYPE</span>
							</div>
						</div>
					</section>
					...
					<div class="run-grid">
						<section class="plan-panel">
							...
							<div class="plan-steps">
								...
								>
									<div class="step-marker">
										...
										<LoaderCircle v-else-if="step.status === 'running'" class="spin" :size="13" />
									...
									<div>
										<div class="step-title">
											...
											<span v-if="step.createdInVersion > 1">v{{ step.createdInVersion }} 新增</span>
										</div>
							...
							<div class="criteria-block">
								...
								>
									<CheckCircle2
										v-if="selectedRun.report?.completedCriteria.includes(criterion)"
										:size="14"
									/>
						...
						<section class="execution-panel">
							<div class="tabs" role="tablist">
								<button
									...
									@click="tab = 'trace'"
								>
									<Activity :size="16" /><span>执行轨迹</span><em>{{ selectedRun.trace.length }}</em>
								</button>
								<button
									role="tab"
								...
								>
									...
									<em>{{ selectedRun.verification.changedPaths.length + selectedRun.verification.deletedPaths.length }}</em>
								</button>
								<button
									role="tab"
							...
							<div v-if="tab === 'trace'" class="trace-scroll">
								...
								>
									...
									<div class="trace-body">
										...
										<p>{{ event.summary }}</p>
										<code v-if="event.toolName" class="tool-chip">{{ event.toolName }}</code>
										<button
										...
										<pre v-if="expandedTraceId === event.id" class="event-data">{{ JSON.stringify(event.data, null, 2) }}</pre>
									</div>
								</div>
								<div v-if="selectedRun.status === 'running'" class="trace-row trace-info">
									<div class="trace-line"><span><LoaderCircle class="spin" :size="14" /></span></div>
									<div class="trace-body waiting-row">
										<strong>Agent 正在决定下一步操作</strong>
										<p>Runtime 会先检查预算和状态，再接收 AI 或 Replay Provider 的决策。</p>
									</div>
								</div>
							</div>
							...
							<div v-else-if="tab === 'diff'">
								<div v-if="!latestDiff" class="panel-empty">
									...
									<p>Agent 修改文件并执行 get_git_diff 后，差异会显示在这里。</p>
								</div>
								<div v-else class="diff-view">
									<div class="changed-files">
										<span v-for="path in selectedRun.verification.changedPaths" :key="path">
							...
							<div v-else-if="tab === 'report'">
								<div v-if="!selectedRun.report" class="panel-empty">
									...
									<p>满足完成条件或触发停止条件后，Runtime 会生成结构化报告。</p>
								</div>
								<div v-else class="report-view">
									<div :class="['report-summary', selectedRun.report.status === 'completed' ? 'success' : 'warning']">
										<CheckCircle2 v-if="selectedRun.report.status === 'completed'" :size="24" />
										<AlertTriangle v-else :size="24" />
										<div>
											<strong>{{ selectedRun.report.status === 'completed' ? '任务已按完成条件交付' : '任务需要继续处理' }}</strong>
											<p>{{ selectedRun.report.summary }}</p>
										</div>
									</div>
									<section class="report-section">
										<h3>已满足的完成条件</h3>
										<div v-for="item in selectedRun.report.completedCriteria" :key="item">
											<CheckCircle2 :size="15" /><p>{{ item }}</p>
										</div>
										<p v-if="selectedRun.report.completedCriteria.length === 0" class="report-empty">尚无完成条件通过</p>
									</section>
									<section class="report-section">
										<h3>剩余问题</h3>
										<div v-for="item in selectedRun.report.remainingIssues" :key="item">
											<AlertTriangle :size="15" /><p>{{ item }}</p>
										</div>
										<p v-if="selectedRun.report.remainingIssues.length === 0" class="report-empty">没有剩余问题</p>
									</section>
									<div class="report-files">
										<div><span>修改文件</span><strong>{{ selectedRun.report.changedPaths.length }}</strong></div>
										<div><span>删除文件</span><strong>{{ selectedRun.report.deletedPaths.length }}</strong></div>
										<div><span>恢复次数</span><strong>{{ selectedRun.usage.recoveryCount }}</strong></div>
									</div>
									<div class="stop-reason"><span>Runtime 结论</span><p>{{ selectedRun.report.stopReason }}</p></div>
								</div>
							</div>
						</section>
					</div>
				...
				<div v-else class="empty-workspace">
					...
					<h1>从一个真实的软件任务开始</h1>
					<p>Agent 会在隔离代码库中分析需求、执行工具、修改文件，并用测试结果决定任务能否结束。</p>
		...
		<div
			v-if="selectedRun?.status === 'waiting_approval' && selectedRun.pendingApproval"
		...
		>
			<div class="approval-dialog" role="dialog" aria-modal="true" aria-labelledby="approval-title">
				...
				<div class="approval-operation">
					...
					<pre>{{ JSON.stringify(selectedRun.pendingApproval.action.arguments, null, 2) }}</pre>
				</div>
				<div class="approval-warning">
					<AlertTriangle :size="16" />
					<p>批准后，Runtime 将在当前隔离工作区执行此操作，并继续运行测试与类型检查。</p>
				</div>
				<div class="dialog-actions">
					<button class="secondary-button" :disabled="approving" @click="approval(false)">
						<X :size="16" /> 拒绝并停止
					</button>
					<button class="danger-button" :disabled="approving" @click="approval(true)">
