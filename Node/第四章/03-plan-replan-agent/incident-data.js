...
const incident = {
	...
	alertContext: {
		...
		summary:
			'支付失败率突然升高。外围告警中出现过 DB_POOL_ACQUIRE_TIMEOUT，值班同学怀疑数据库连接池异常，但还没有核验。'
	},
	metrics: {
		...
		summary:
			'5xx 和响应延迟在 15:10 同时升高，但 CPU、内存和数据库连接池均处于正常范围。当前指标不支持连接池耗尽假设。'
	},
	databasePool: {
	...
	logs: {
		...
		summary:
			'主要错误来自 currency 字段读取失败，首次出现时间为 15:10:08，错误实例均运行 v2.4.1。'
	},
	deployments: {
		items: [
			{
		...
		summary:
			'v2.4.1 在异常出现前约 24 秒完成发布，并修改了发生报错的币种归一化逻辑。'
	}
}
